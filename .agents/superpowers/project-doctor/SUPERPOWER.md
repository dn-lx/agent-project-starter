...
4. Check that non-trivial work has a usable planning path, tests are meaningful rather than nominal, material frontend work has rendered-runtime verification, and performance/analytics contracts are defined when the product uses them.
...
7. Measure startup context with `scripts/context-budget.mjs`.
